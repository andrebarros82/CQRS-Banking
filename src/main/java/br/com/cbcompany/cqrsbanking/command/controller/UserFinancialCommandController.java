...
/**
 * Controlador responsável por expor os endpoints de comandos relacionados a
 * operações financeiras do usuário, como depósito, saque e pagamento de contas.
 *
 * <p>
 * Este controlador segue o padrão CQRS, delegando a execução dos comandos ao
 * {@link UserFinancialCommandService}, enquanto as consultas são realizadas por
 * outros componentes do sistema.</p>
 *
 * <p>
 * Todos os endpoints exigem autenticação JWT para identificar o usuário.</p>
 *
 * @author andre
 */
@RestController
...
public class UserFinancialCommandController {

    /**
     * Serviço responsável por executar os comandos financeiros.
     */
    private final UserFinancialCommandService userFinancialCommandService;

    /**
     * Construtor do controlador.
     *
     * @param userFinancialCommandService serviço de comandos financeiros.
     */
    public UserFinancialCommandController(UserFinancialCommandService userFinancialCommandService) {
        this.userFinancialCommandService = userFinancialCommandService;
    }

    /**
     * Realiza a operação de depósito de um valor monetário na conta do usuário
     * autenticado.
     *
     * @param request objeto contendo o valor do depósito.
     * @param authentication contexto de autenticação que fornece o login do
     * usuário.
    ...
    @Operation(
            ...
            responses = {
                ...
                @ApiResponse(
                        responseCode = "400",
                        description = "Dados inválidos ou saldo inconsistente",
    ...
    public DepositResponseDTO deposit(
        ...
        return userFinancialCommandService.deposit(login, request);
    }

    /**
     * Realiza a operação de saque de um valor monetário da conta do usuário
     * autenticado.
     *
    ...
    public WithdrawResponseDTO withdraw(
        ...
        String login = authentication.getName();
        return userFinancialCommandService.withdraw(login, request);
    }

    /**
     * Realiza a operação de pagamento de contas utilizando o saldo do usuário
     * autenticado.
     *
    ...
    public PayBillResponseDTO payBill(
        ...
        String login = authentication.getName();
        return userFinancialCommandService.payBill(login, request);
    }
}
